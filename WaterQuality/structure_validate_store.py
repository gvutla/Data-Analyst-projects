...
for msg in consumer:
    if msg.value.decode("utf-8") != "Error in Connection":
        ...
        if timestamp_exist(data['TimeStamp']) == False:            
            ...
            # Push data to ValidatedSensorData Kafka topic
            producer.send("ValidatedSensorData", json.dumps(data, default=json_util.default).encode('utf-8'))
            # Push data to CleanSensorData Kafka topic
